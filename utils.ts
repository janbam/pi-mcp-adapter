...
/**
 * FORK-OWNED (janbam/pi-mcp-adapter): PI_MCP_TIMEOUT env override for MCP
 * request timeouts.
 *
 * Upstream resolves timeouts solely from the optional global/per-server
 * `requestTimeoutMs` config keys and otherwise lets the MCP SDK default apply.
 * This fork adds an opt-in env knob: when `PI_MCP_TIMEOUT` is set,
 * `server-manager.ts` uses it as the request timeout and enables
 * `resetTimeoutOnProgress` so progress notifications extend the window. When
 * unset, behavior is byte-identical to upstream. Preserve across upstream syncs.
 *
 * Value: positive integer milliseconds; invalid values fail loud on first use.
 * Returns undefined when the variable is unset.
 */
let cachedCallTimeoutMs: number | undefined;
let callTimeoutResolved = false;

export function getCallTimeoutMs(): number | undefined {
  if (callTimeoutResolved) return cachedCallTimeoutMs;
    ...
    if (!Number.isSafeInteger(parsed) || parsed <= 0 || String(parsed) !== raw.trim()) {
      throw new Error(
        `Invalid PI_MCP_TIMEOUT="${raw}": expected a positive integer (milliseconds). ` +
          `Unset it to use the MCP SDK default timeout.`,
      );
    }
    cachedCallTimeoutMs = parsed;
  }
  callTimeoutResolved = true;
