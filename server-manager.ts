...
import { getCallTimeoutMs } from "./utils.ts";
...
export class McpServerManager {
  ...
  private buildRequestOptions(
    ...
    // FORK-OWNED: opt-in PI_MCP_TIMEOUT env knob overrides the SDK default when
    // no per-server/global requestTimeoutMs is configured; progress notifications
    // extend the window.
    const effectiveTimeout = timeout ?? getCallTimeoutMs();
    const fromEnv = timeout === undefined && effectiveTimeout !== undefined;

    if (!ownedSignal && effectiveTimeout === undefined) {
      return undefined;
    }
    ...
    return {
      ...
      ...(fromEnv ? { resetTimeoutOnProgress: true as const } : {}),
    };
  }
