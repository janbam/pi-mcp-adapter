...
| `description` | Short summary of what the server does, shown to the model in the `mcp__<server>` namespace proxy and in the `mcp` gateway's `Servers:` list, so it can judge relevance without searching first. Whitespace and newlines collapse to single spaces. Editing it does not invalidate cached tool metadata. |
...
|----------|-------------|
| `PI_MCP_TIMEOUT` | Optional override for the MCP request timeout in milliseconds. When set to a positive integer, it applies wherever no global or per-server `requestTimeoutMs` is configured, and server progress notifications reset the timer. Unset leaves timeout behavior identical to upstream (MCP SDK default); invalid values fail loud on first tool call. |
